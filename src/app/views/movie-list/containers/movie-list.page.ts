...
import { MovieListPageState } from '../model';
...
@Component({
  ...
  <ion-content [fullscreen]="true" [scrollEvents]="true" (ionScroll)="logScrolling($any($event))">
    ...
    <div class="empty-header components-color">
      ...
      <h1 class="text-colorr">{{'COMMON.MOVIES' | translate }} {{ parseTitle(this.typeMovie) }}</h1>
          ...
          <ng-container *ngIf="status !== 'pending' || componentStatus?.page > 1; else loader">
            <ng-container *ngIf="status !== 'error'; else serverError">
              <ng-container *ngIf="info?.list?.length > 0; else noData">
                ...
                <app-item-card
                  *ngFor="let movie of info?.list; trackBy: trackById"
                  [item]="movie"
                  [from]="'movie'">
                </app-item-card>
...
export class MovieListPage {
  ...
  status$ = this.route.params.pipe(
    switchMap(({typeMovie}) =>
      this.store.select(fromMovie.selectListStatus(typeMovie)).pipe(
        map((status) => status || EntityStatus.Pending)
      )
    ),
  );

  trigger = new EventEmitter<MovieListPageState>();
  componentStatus: MovieListPageState;
  info$ = this.trigger.pipe(
    concatLatestFrom(() => this.store.select(fromMovie.selectListMovies)),
    // tap(d => console.log(d)),
    tap(([{typeMovie, page, filter, reload}, storeData]) => {
      if(!reload){
        const { filter:storeFilter = null } = storeData?.[typeMovie] || {};
      ...
      if((!storeData?.[typeMovie]?.list || storeData?.[typeMovie]?.list?.length === 0) || page > 1 || !!reload){
  ...
  ionViewWillEnter(): void{
    this.search.reset();
    this.typeMovie = this.route.snapshot.params?.typeMovie || 'popular';
    this.componentStatus = {
